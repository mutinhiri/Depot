class Product1sController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @product1.save
        format.html { redirect_to product1_url(@product1), notice: "Product1 was successfully created." }
  ...
  def update
    respond_to do |format|
      if @product1.update(product1_params)
        format.html { redirect_to product1_url(@product1), notice: "Product1 was successfully updated." }
  ...
  private
    ...
    def product1_params
      params.require(:product1).permit(:title, :description, :image_url, :price)
    end
end
